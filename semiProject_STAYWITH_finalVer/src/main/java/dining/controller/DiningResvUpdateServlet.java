...
/**
 * Servlet implementation class DiningResvUpdateServlet
 */
@WebServlet(name = "DiningResvUpdate", urlPatterns = { "/diningResvUpdate" })
public class DiningResvUpdateServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public DiningResvUpdateServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session != null) {
			...
			int result = new DiningService().updateDiningResv(dr);
			if(result>0) {
				request.setAttribute("msg", "예약이 변경되었습니다.");
			}else {
