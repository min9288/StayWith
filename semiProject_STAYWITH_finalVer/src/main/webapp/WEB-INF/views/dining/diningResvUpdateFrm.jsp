...
<body>
	...
	<div class="main">
            <h2>다이닝 예약 변경</h2>
            <form action="/diningResvUpdate" method="post">
            <input type="hidden" name="resNo" value="${dr.resNo }">
                <div id="name-box">
                    <label for="diningNo" class="col-sm-2 col-form-label">다이닝 선택 <span class="req">*</span></label>
                    <div class="col-sm-6">
                        <select name="diningNo" id="diningNo" class="form-select" disabled>
                            <option value="${dr.diningNo}">${dr.diningName }</option>
                        </select>
                ...
                <div class="date-time-box">
                    <div class="date-box">
                        ...
                        <input type="hidden" id="oldTimeType" value="${dr.timeType }">
                        <input type="hidden" id="oldDate" value="${dr.RDate }">
                        <input type="hidden" name="rDate" value="${dr.RDate }">
                        <div id="calendar"></div>
                    </div>
                    <div class="time-box">
                        ...
                        <div class="btn-group timeType" role="group" aria-label="Basic radio toggle button group">
                        	<c:choose>
                        		<c:when test="${dr. timeType eq 1}">
		                            <input type="radio" class="btn-check" name="timeType" id="timeType1" value="1" checked>
                        		...
                        		<c:otherwise>
		                            ...
		                            <label class="btn btn-outline-secondary" for="timeType1">런치</label>
		                            <input type="radio" class="btn-check" name="timeType" id="timeType2" value="2" checked>
		                            <label class="btn btn-outline-secondary" for="timeType2">디너</label>
                        		</c:otherwise>
                        	</c:choose>
                        </div>
                        <div class="col-sm-12">
                        	<input type="hidden" value="${dr.resTime }" class="oldTime">
                            <select name="resTime" id="resTime" class="form-select">
                                <option value="" class="no">방문 예정 시간을 선택해주세요.</option>
                            </select>
                            <p>※ 방문 시간 변경은 기존 예약일의 온라인 예약이 마감되었더라도 기존 예약 시간대에 한해 가능합니다.</p>
                        </div>
                    </div>
                </div>
                <div class="seat-box">
                    ...
                    <div class="btn-group seatType" role="group" aria-label="Basic radio toggle button group">
                        <c:choose>
                        		<c:when test="${dr. seatType eq 1}">
		                            <input type="radio" class="btn-check" name="seatType" id="seatType1" value="1" checked>
                        		...
                        		<c:otherwise>
			                        ...
			                        <label class="btn btn-outline-secondary" for="seatType1">테이블</label>
			                        <input type="radio" class="btn-check" name="seatType" id="seatType2" value="2" checked>
			                        <label class="btn btn-outline-secondary" for="seatType2">룸</label>
                        		</c:otherwise>
                        	</c:choose>
                    </div>
                ...
                <div id="guest" class="guest-box row">
                    ...
                    <div>
                        <div class="guest-num">
                            ...
                            <input type="text" name="adtCnt" readonly value="${dr.adtCnt }">
                            <button type="button" class="btn btn-light btn-sm">＋</button>
                        </div>
                        <div class="guest-num">
                            <span>어린이</span>
                            <button type="button" class="btn btn-light btn-sm">－</button>
                            <input type="text" name="kidCnt" readonly value="${dr.kidCnt }">
                            <button type="button" class="btn btn-light btn-sm">＋</button>
                        </div>
                        <div>
                            <p>※ 어린이 : 만 11세 이하</p>
                ...
                <div class="btn-box">
                    <button type="submit" class="btn btn-secondary" onclick="return check();">예약 변경</button>
                </div>
            </form>
        </div>
        <div class="modal">
			<div class="modal-dialog" role="document">
		    	<div class="modal-content">
		    		...
		    		<div class="modal-footer">
			        	...
			        	<button type="button" class="btn btn-primary modal-submit">예약 변경 완료</button>
		    		</div>
		  		</div>
			</div>
		</div>
...
<link rel="stylesheet" href="/css/diningResvUpdateFrm.css">
<script type="text/javascript" src="/js/diningResvUpdateFrm.js"></script>
<!-- Calendar 추가 함수 -->
